...
class Substitution(Module):
    def __get_substitution_data(self, date: date) -> str:
        url = (
            f"https://{self.edupage.subdomain}.edupage.org/substitution/server/viewer.js"
            "?__func=getSubstViewerDayDataHtml"
        )

        data = {
            "__args": [None, {"date": date.strftime("%Y-%m-%d"), "mode": "classes"}],
    ...
    def get_missing_teachers(self, date: date) -> Optional[list[EduTeacher]]:
        ...
        try:
            ...
            raise InvalidTeacherException(
                "Invalid teacher in substitution! "
        ...
        for class_changes in changes:
            ...
            for change in class_changes_rows:
                ...
                if "-" in lesson_n:
                    ...
                    lesson_n = (
                        ModuleHelper.parse_int(lesson_from),
