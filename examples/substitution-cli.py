...
parser = argparse.ArgumentParser(
    description="Get substitution information from edupage."
)
parser.add_argument("--output", metavar="file", type=str, default="stdout")
parser.add_argument(
    "--format", metavar="format", choices=["json", "text", "csv"], default="text"
)
parser.add_argument(
    "--date",
    metavar="date",
...
def get_text(
    missing_teachers: Optional[list[EduTeacher]],
    timetable_changes: Optional[list[TimetableChange]],
):
    s = ""
...
if args.format == "text":
    ...
    if args.format == "json":
        ...
        if timetable_changes:
            ...
            timetable_changes_writer = csv.DictWriter(
                stream, fieldnames=["change_class", "lesson_n", "title", "action"]
            )
